...
- **`src/components/OpenRouterModelBrowser.tsx`** — the OpenRouter model picker: search, provider chips, sort, and model cards over OpenRouter's public `/api/v1/models` catalog (fetched key-less by `fetchOpenRouterCatalog()` in `aiClient.ts`, trimmed/filtered/sorted by `src/data/openrouterCatalog.ts`, cached 24h in `localStorage` under `loresieve_openrouter_catalog`). `OPENROUTER_MODELS` appears under its "★ Recommended" chip. Any lowercase catalog ID the user picks is a deliberate choice: `migrateModel()` must only remap capitalized pre-overhaul values, never rewrite a pinned pick to a `latest` alias.

## Repo / workflow notes

- The owner has **zero coding background**. Explain things plainly.
...
- **Branch policy (owner decisions — follow exactly):**
  - `main` is the **uncensored local build**. It must **never** contain the `AgeGate` component, the `SAFETY_REFUSAL` prompt block, or the `{"refusal": ...}` detection in `aiClient.ts`. A regression test (`main carries no age gate or content refusal rule`) enforces this; do not delete or weaken it. If any of those ever appear on `main` (e.g. via a mistaken merge), strip them.
  - **Everything is pushed straight to `main`.** Once tests, lint, and build pass, merge the work into `main` and push it in the same session — do not leave finished work sitting on a `claude/*` branch or waiting on a PR the owner has to merge. `claude/*` working branches are temporary scratch space that track **`main`**, never `public-release`; base them on `main` and never merge `public-release` into them. The same "never" list applies to them.
  - `public-release` is the **store/public build**: `main` plus the `AgeGate` first-launch 18+ overlay, the `SAFETY_REFUSAL` constant that `buildPrompt` prepends to every prompt (full and token-efficient alike — never inside either rubric, so the toggle can't drop it), and the refusal detection in `aiClient.ts` `run()` (must run before `normalizeResult`, which would otherwise reject the refusal shape as an invalid report). **Do not update, merge into, or push `public-release` unless the owner explicitly asks for it in that session.** When asked, the direction is always `main` → `public-release`, never the reverse.
  - There is no in-app "report output to the developer" feature (a `ReportOutputButton`/mailto button existed briefly and was removed after it shipped the owner's personal email into the public bundle). **Never hardcode the owner's personal email or any personal PII anywhere in the source** — this repo is public with GitHub Pages auto-deploying `main` on every push, so anything hardcoded here is instantly public, in the source *and* the shipped bundle. If a feedback/report mechanism is wanted again, it needs a dedicated project/support address the owner explicitly supplies for that purpose — never infer one from ambient user context.
- **Version number is real.** The header's `Auditor vX.Y.Z` and the footer's `ENGINE_vX.Y.Z // BUILD_<sha>` come from `package.json` `version` and the git commit (injected via `define` in `vite.config.ts`). **Bump the patch version in `package.json` on every push to `main` that changes what the user sees or how grading works** (`npm version patch --no-git-tag-version`) — the owner uses it to confirm an update actually landed. Never hardcode a version string in the UI.
