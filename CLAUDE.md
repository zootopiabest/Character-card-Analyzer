...
- **Branch policy (owner decisions — follow exactly):**
  ...
  - `claude/*` working branches track **`main`**, never `public-release`. Base new work on `main`, keep the branch in sync with `main`, and never merge `public-release` into it. The same "never" list applies to them.
  ...
  - The `ReportOutputButton` (mailto feedback) lives on both lines; it is not part of the censorship layer.
