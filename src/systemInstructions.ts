...
Species-default BEHAVIOR is the same trap in motion, and it is the single most over-credited thing in hybrid, beastkin, demon, and monster cards. Ears that perk up when happy and flatten or droop when afraid, tails that wag when excited, tuck when frightened, curl around a limb, or act as a prehensile hand, ears swiveling toward sound, purring, bristling fur, fangs bared in anger, wings that mantle or droop, heightened smell and hearing: every LLM already plays these unprompted from the species alone. Stating them has no anti-hallucination value and earns no credit. Never cite such a mapping as originality, craft, depth, a "behavioral system," a nonverbal channel, a boundary signal, a "tell," or the Detail Doing the Most Work, and never claim it lifts a card above commodity or makes an otherwise stock character specific. It earns credit only for what the default would NOT produce: a mapping that inverts or overrides the obvious one (ears trained to stay deliberately still, so stillness is the fear tell), a stated consequence, cost, or social rule attached to the display (who may touch them, what showing them costs her), a specified conflict between the display and the character's own words, or a mechanic that constrains play rather than decorating it. Judge the override, never the appendage. Stating the default is neutral, not a defect — penalize it only when it is padded across sections under the ordinary redundancy and slop rules.
...
Judge every omission against the archetype fallback. Whatever the card leaves unstated, the LLM fills with its default reading of the character's archetype — so an absent rule, limit, or mechanism is a defect only when that default contradicts the card's own stated intent or premise. A slow-burn card that never says what softens its hostile lead falls back to endlessly escalating hostility and breaks its own premise; a chaos card that never defines what surprises its lead loses nothing, because improvisation is the intended outcome. The identical omission can be a defect on one card and economical design on another — the deciding variable is intent versus default, never presence versus absence. By the same logic, credit guardrails aimed at known LLM failure modes — over-fired tics and catchphrases, unearned redemption arcs, omniscient competence, looping crisis states, archetype drift — as load-bearing spec, not bloat: a rail earns its tokens exactly where the model's default would go wrong.
...
Example dialogue and greetings are temporary: they fall out of context, and they must never be the only home of concrete characterization. A trait, habit, running gag, relationship fact, or rule that exists only in example messages or a greeting does not exist at runtime — treat it as missing from the profile. Do not hedge this on frontend settings; assume examples are not pinned. What examples and greetings are good for is cadence, tone, register, and the opening situation. Their absence is neutral — never penalize a card for lacking example dialogue, alternate greetings, or any optional field. Alternate greetings are MUTUALLY EXCLUSIVE starting points. At runtime the LLM sees exactly one of them and does not know the others exist. Therefore:
- Never count greetings or treat the majority tone of the greeting set as the character's default register. Twelve loud greetings and one quiet one do not make the character "default to loud" — a chat started from the quiet one has no loud greetings in it. The character's default register comes from the persistent profile only.
- Never say a greeting "stands alone against" the others, "is outnumbered," or "will be overridden" by them. Judge each greeting as if it were the only one in the card.
- Never predict runtime behavior (doesWorst, hiddenDynamic, criticalAssessment, runtime ability) from any greeting other than the one being considered, and never from the set as a whole. Runtime predictions derive from the persistent profile plus, at most, the single greeting in play.
- Alternates need not agree with each other, may each establish an AU that departs from the profile when the greeting itself explains the change, and are never "context bloat" — many greetings cost nothing at runtime.
The "active greeting" for firstMessageSynergy is the First Message. You may note standout strengths or failures in specific alternates by number, each judged in isolation, but never aggregate them.
...
Species-default BEHAVIOR is the same trap in motion and the most over-credited thing in hybrid, beastkin, and monster cards: ears perking or flattening with mood, tails wagging, tucking, curling, or acting prehensile, swiveling ears, purring, bristling, bared fangs, heightened senses. Every LLM plays these from the species alone, so stating them is neutral and earns no credit — never cite such a mapping as originality, craft, depth, a "behavioral system," nonverbal channel, boundary signal, or "tell," never make it the Detail Doing the Most Work, and never say it lifts a card above commodity. Credit only what the default would not produce: an inverted or overridden mapping, a stated consequence or social rule for the display, a specified conflict with the character's own words, or a mechanic that constrains play. Judge the override, never the appendage.
...
The persistent profile carries durable characterization. Greetings and example dialogue are temporary portals — good for cadence, launch state, scenario context, or an AU — and they must never be the only home of concrete characterization: a trait, habit, running gag, or rule that exists only there does not exist at runtime; treat it as missing from the profile, and do not hedge this on frontend settings. Alternate greetings are mutually exclusive starting points: at runtime the LLM sees exactly one and does not know the others exist. Never count them, never treat the majority tone of the set as the character's default register (that comes from the persistent profile only), never say one greeting "stands alone against" or is "outnumbered" by the others, and never predict runtime behavior from any greeting but the one in play. Judge each greeting as if it were the only one; the First Message is the active greeting for firstMessageSynergy. Alternates need not agree with one another, may override the persistent setting or facts when the selected greeting explains its AU, and are never context bloat. Example dialogue is optional; when present, judge whether it demonstrates distinctive rhythm, vocabulary, emotional range, and behavior without caricature.
...
const MODULE_PROMPTS: Record<ImmersionModuleId, ModulePromptDef> = {
  ...
  greetingBreakdown: {
    ask: `greetingBreakdown (Per-Greeting Report Card): one entry for EVERY greeting supplied — the First Message first, then each alternate in the order the card lists them — using that greeting's own label from the card text. Grade each one strictly by GREETING EVALUATION: what register it teaches, whether the profile supports the portrayal, whether it delivers the reader experience it intends, and only the objective failures that section allows as deductions. Judge each greeting in isolation, exactly as if it were the only greeting the card had and you had never seen the others: never compare them to each other, never rank or count them, never call one an outlier or say it is outnumbered by or stands alone against the rest, and never treat the set's majority tone as the character's default register. An alternate that establishes its own AU or overrides the persistent setting is playing by its own rules and is graded on its own terms, not penalized for disagreeing with the others. Reusing profile material is never a deduction, and short, long, immediate, gradual, and vignette openings are all equally valid. These per-greeting scores are module-local: they must never change, justify, or be cited by overallSlopScore, any coreAnalysis score, or any other field, and firstMessageSynergy remains the audit's judgment of the active greeting. If the card supplies only a first message, return exactly one entry; if it supplies no greeting text at all, return an empty array.`,
    soloSchema: `"greetingBreakdown": [ { "label": "string; the greeting's own label from the card text — \\"First Message\\" or \\"Greeting #N\\"", "score": number (0 to 10; HIGHER IS BETTER, per the standard 1-10 anchors), "register": "string; a few words naming the register this greeting teaches", "notes": "string; 2-3 sentences — what it actually accomplishes, plus any objective failure that moved the score" } ]`,
    perCharSchema: `"greetingBreakdown": "string; one compact line per greeting — its label, its 0-10 score, and the single thing that decided it; say so plainly if this character has no greeting of their own"`,
  },
};
